...
void measure(const std::vector<double>& v, std::size_t start = 16, std::size_t count = 500)
{
	Timer t;
	...
	{
		t.resume();
		result = parallel_accumulate(std::begin(v), std::end(v), start + i, 0.0);
		t.pause();
...
int main()
{
	...
	{ 
		measure(v); 
		std::cout << "The most optimal count of currents equals " << optimize() << std::endl;//17
	}
	catch (const std::runtime_error& exception)
	{
		std::cerr << "Runtime exception: " << exception.what() << " in " << __LINE__ << " line" <<  std::endl;
	}
